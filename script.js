...
const produtos = [
  ...
  //Seleção do produto a comprar
    ...
    var regiao = document.querySelector('input[name="localização"]:checked');
    ...
    if (talão == 1) {
        ...
        if (isNaN(NIF) || NIF.length != 9 || parseInt(NIF) !== parseFloat(NIF)) {
          alert('Insira um valor válido (sequência numérica de 9 algarismos).');
          return;
        }
      ...
      if (troco >= 0) {
        ...
        if (avariar == 0) {
          ...
          desativarl.disabled = true;
          desativarl1.disabled = true;
          desativarl2.disabled = true;
          return;
        }
        else {
      ...
      if (troco >= 0) {
        ...
        //A máquina avariou
        if (avariar == 0) {
          alert('Ups!! Parece que a máquina avariou! Dica: tente dar pancadas na máquina de forma a consertá-la.');
  ...
  function pancada() {
    ...
    if (pancada == 0) {
      ...
      desativarl.disabled = false;
      desativarl1.disabled = false;
      desativarl2.disabled = false;
    }
    else {
